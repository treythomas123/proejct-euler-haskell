...
isLychrel n = all (not . isPalindromic) $ tail $ take 50 $ scanl (\ a _ -> addReverse a) n [1..]
    where addReverse x = ((read::String->Integer) $ reverse $ show x) + x

main = print $ length $ filter isLychrel [10..9999]
